...
import {examinerApi} from './api';
import {Layout} from '../layout';

export const Examiners = () => {
  const {data: allExaminers} = examinerApi.endpoints.getExaminers.useQuery(undefined, {pollingInterval: 10000});
  const {data: self} = examinerApi.endpoints.getSelf.useQuery();
  const [deleteExaminer] = examinerApi.endpoints.deleteExaminer.useMutation();
  return <Layout>
